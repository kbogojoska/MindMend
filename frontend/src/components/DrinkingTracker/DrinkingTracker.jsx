...
function DrinkingTracker() {
  ...
  const loadDrinkingData = async () => {
    setLoading(true);
    try {
      const result = await axios.get('http://localhost:8080/api/drinking-tracker');
      ...
      setErrors((prevState) => ({
        ...prevState,
        connectionErrorLoadingData:
          "There was an error loading the drinking trackers",
      }));
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async (id, event) => {
    ...
    try {
      await axios.delete(
        `http://localhost:8080/api/drinking-tracker/delete/${id}`
      );
      loadDrinkingData();
    } catch (error) {
      setErrors((prevState) => ({
        ...prevState,
        connectionErrorOnDelete:
          "There was an error deleting the drinking tracker",
      }));
    } finally {
      setLoading(false);
    }
          ...
          >
            ...
            {axiosData.length === 0 && (
              <div className="justify-content-center align-items-center m-2">
                <h5>
                  There are currently no Drinking Trackers. Create one on the plus
                  sign!
                </h5>
              </div>
            )}
              ...
              {axiosData.map((element, index) => (
                ...
                >
                  <DrinkingTrackerItem
                    ...
                    numOfDrinks={element.numOfDrinks}
                    maxDrinks={element.maxDrinks}
                  />
...
export default DrinkingTracker;
